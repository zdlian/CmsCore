...
namespace CmsCore.Model.EntityBuilders
{
    ...
    {
        public ProductCategoryBuilder(EntityTypeBuilder<ProductCategory> entityBuilder)
        {
            ...
            entityBuilder.HasOne(p => p.Language).WithMany(l => l.ProductCategories).HasForeignKey(p => p.LanguageId).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
