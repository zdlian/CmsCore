var initTable1 = function () {
    ...
    table.dataTable({
        ...
        "bProccessing": true,
        "sAjaxSource": "/Template/AjaxHandler",
...

            {
                ...
                'render': function (data, type, row) {
                    return '<div class="btn-group"><button class="btn btn-xs green dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false">Eylemler<i class="fa fa-angle-down"></i></button>'
                        + '<ul class="dropdown-menu" role="menu"><li><a href="/Template/Edit/' + row[0] + '"><i class="icon-note"></i> Düzenle</a></li><li><a href="/Template/Details/' + row[0] + '"><i class="icon-list"></i> Detaylar</a></li><li>'
                      + '<a href="/Template/Delete/' + row[0] + '" onclick="if (!confirm(\'Bu kaydı silmek istediğinize emin misiniz? Bu işlem geri alınamaz.\')) return false;"><i class="icon-ban"></i> Sil</a></li></ul></div>';
                }
    ...
    var tableWrapper = jQuery('#templatesTable_wrapper');
